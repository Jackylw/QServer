/**
 * @author Jacky Feng
 * @date 2024/3/17 16:46
 */
package top.fexample.qchat.common;

public interface MessageType {
    String CONNECT_SERVER_TIMEOUT = "CONNECT_SERVER_TIMEOUT";
    String LOGIN_SUCCESS = "LOGIN_SUCCESS";
    String LOGIN_FAIL = "LOGIN_FAIL";
    String LOGIN_REPEAT = "LOGIN_REPEAT";
    String REGISTER_EXIST = "REGISTER_EXIST";
    String REGISTER_SUCCESS = "REGISTER_SUCCESS";
    String REGISTER_ERROR = "REGISTER_ERROR";
    String COMMON_MESSAGE = "COMMON_MESSAGE";
    String GET_ONLINE_FRIEND = "GET_ONLINE_FRIEND";
    String RECEIVE_ONLINE_FRIEND = "RECEIVE_ONLINE_FRIEND";
    String REQUEST_FRIEND = "REQUEST_FRIEND";
    String RECEIVE_FRIEND_REQUEST = "RECEIVE_FRIEND_REQUEST";
    String CLIENT_EXIT = "CLIENT_EXIT";
    String ADD_USER = "ADD_USER";
    String ADD_USER_SUCCESS = "ADD_USER_SUCCESS";
    String ADD_USER_FAIL = "ADD_USER_FAIL";
    String DELETE_USER = "DELETE_USER";
    String DELETE_USER_SUCCESS = "DELETE_USER_SUCCESS";
    String DELETE_USER_FAIL = "DELETE_USER_FAIL";
    String FIND_USER_NOT_EXIST = "FIND_USER_NOT_EXIST";
    String FIND_USER_EXIST = "FIND_USER_EXIST";
    String MODIFY_PASSWORD_SUCCESS = "MODIFY_PASSWORD_SUCCESS";
    String MODIFY_PASSWORD_FAIL = "MODIFY_PASSWORD_FAIL";
}